...
const sourceWarning = (error) => ({
  ...
  warnings: [{
    ...
    errorMessage: clean(error?.message || "SMS draft storage unavailable.", 240),
  }],
});
...
exports.handler = async (event) => {
  try {
    ...
    if (event.httpMethod === "GET") {
      try {
        const drafts = await list("sms_drafts", { caseId: params.caseId, status: params.status, limit: params.limit });
        return json(200, { ok: true, drafts, sourceUnavailable: false, storageAvailable: true, warnings: [] });
      } catch (error) {
        return json(200, { ok: true, drafts: [], ...sourceWarning(error) });
      }
    }
    ...
    console.error("sms-drafts failed", {
      name: error?.name,
      message: error?.message,
      method: event?.httpMethod,
    });
    return json(500, {
      error: "SMS draft storage unavailable",
      code: error?.name || "SMS_DRAFTS_ERROR",
      ...sourceWarning(error),
    });
  }
};
